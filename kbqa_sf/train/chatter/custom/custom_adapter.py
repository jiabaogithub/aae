...
class BestMatchExtLogicAdapter(LogicAdapter):
    ...
    def get(self, input_statement):
        ...
        indx_for_confidence_tuple = np.where(confidence_vec >= current_app.config[
            'THRESHOLD_SEMANTICS_UNDERSTANDER_TALK'])  # 筛选出大于阈值的值得索引,返回的是一个元组，元组只有一个元素是一个索引组成的ndarray
        if len(indx_for_confidence_tuple) == 0:
            return closest_match
        indx_for_confidence = indx_for_confidence_tuple[0]
        if indx_for_confidence.size == 0:
            logging.debug("talk领域语义理解在正常置信度阈值下没有匹配到任何项，进行降级匹配...")
            indx_for_confidence_tuple = np.where(confidence_vec >= current_app.config[
                'THRESHOLD_SEMANTICS_UNDERSTANDER_TALK_MIN'])  # 降级筛选,返回的是一个元组，元组只有一个元素是一个索引组成的ndarray
            indx_for_confidence = indx_for_confidence_tuple[0]
        ...
        #     if confidence >= current_app.config['THRESHOLD_SEMANTICS_UNDERSTANDER_TALK']:
        #         statement.confidence = confidence
        #         closest_match = statement
        #         closest_match_list.append(statement)
    ...
    def process(self, input_statement):
        ...
        for closest_match in list_:
            # 若匹配到的是答案，就返回当前答案对象自身做为一个响应对象(目前上游查询逻辑改成了只查询问题，所以不会查询出答案。此if语句中的代码块将不会有机会执行)
